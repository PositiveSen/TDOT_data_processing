...
class EnhancedUniversalTDOTModel:
    ...
    def train_enhanced_model(self, target_categories: List[str] = None) -> Tuple[CatBoostRegressor, float]:
        ...
        if target_categories is None:
            # Only categories we have CSV files for: 300s, 400s, and 716
        ...
        all_features = core_features + classification_features + project_features + text_feature_cols + all_embedding_cols
        ...
        print(f"  Project MPNet embeddings: {len(project_embedding_cols)} (NEW)")
        print(f"  Total features: {len(all_features)}")
        print(f"  🎯 Testing if project description context enhances price prediction accuracy")
        ...
        # Disabled text features for testing
        cat_features = [
    ...
    def _plot_residuals(self, y_real, y_pred, dataset_name, plots_dir, suffix=""):
        ...
        # Residuals vs predicted - both in normal scale for consistency
        ax1.scatter(y_pred, residuals, alpha=0.5, s=20)
        ax1.axhline(y=0, color='red', linestyle='--')
        ax1.set_xlabel('Predicted Unit Price ($)')
        ax1.set_ylabel('Residuals ($)')
        ax1.set_title(f'Residuals vs Predicted - {dataset_name}')
        # Remove log scale to keep both axes consistent in normal scale
        ax1.grid(True, alpha=0.3)
        
        # Relative error histogram
        ax2.hist(relative_error, bins=50, alpha=0.7, edgecolor='black')
    ...
    def _analyze_grouped_feature_importance(self, model, all_features):
        ...
        for i, feature in enumerate(all_features):
            ...
            elif feature in classification_features:
                grouped_importance['Classification Features'] += importance
                detailed_groups['Classification Features'].append((feature, importance))
...
def main():
    ...
    # Train with only categories we have CSV files for (300s, 400s, 716)
    target_categories = ['302', '303', '401', '402', '403', '404', '405', '406', '407', '408', '409', '716']
    ...
    print(f"Categories included: 300s, 400s, and 716 only")
    print(f"Using Item Class and Item Type instead of hardcoded category mappings")
