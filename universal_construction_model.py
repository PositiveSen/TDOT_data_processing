#!/usr/bin/env python3
"""
Universal TDOT Construction Price Prediction Model
Expands proven pavement marking methodology to concrete, aggregates, and other categories
"""
...
def load_description_corrections():
    """Load description correction rules"""
    return {
        "description_corrections": {
            "abbreviations": {
                "PVMT": "PAVEMENT", "MRKNG": "MARKING", "CONC": "CONCRETE", 
                "AGGR": "AGGREGATE", "ASPH": "ASPHALT", "CY": "CUBIC YARD",
                "SY": "SQUARE YARD", "LF": "LINEAR FOOT", "SF": "SQUARE FOOT",
                "THERMO": "THERMOPLASTIC", "ALUM": "ALUMINUM"
            },
            "regex_patterns": [
                {"pattern": r"\b(\d+)\s*IN\b", "replacement": r'\1"', "description": "inches"},
                {"pattern": r"\b(\d+)\s*FT\b", "replacement": r'\1 FOOT', "description": "feet"},
                {"pattern": r"\s+", "replacement": " ", "description": "normalize spaces"}
            ]
        },
...
def universal_construction_model():
    """
    Universal construction price prediction model
    Expands proven pavement marking methodology to concrete, aggregates, and other categories
    """
    print("=== UNIVERSAL TDOT CONSTRUCTION PRICE PREDICTION ===")
    print("Expanding proven methodology to multiple construction categories")
    ...
    print("403 = Category 403 (new addition)")
    ...
    # Data cleaning (same as pavement marking model)
    ...
    # Temporal features (same as pavement marking)
    ...
    # Target and quantity features (same transformation)
    ...
    # Train/test split (same as pavement marking)
    print(f"\n" + "="*80)
    ...
    core_features = [
        'item_description_expanded', 'category_name', 'Primary County',
    ...
    all_features = core_features + text_feature_cols
    ...
    print(f"\n" + "="*80)
    print("TRAINING UNIVERSAL CONSTRUCTION MODEL")
    print("="*80)
    ...
    # Train model (same hyperparameters as proven successful)
    ...
    for i, category in enumerate(test_data['category_name'].unique()):
        ...
        if cat_mask.sum() > 10:  # Only show categories with enough data
            # Get predictions and actual values for this category
    ...
    for _, row in importance_df.head(20).iterrows():
        marker = "🏗️" if row['feature'] in text_feature_cols else "📊"
    ...
    try:
        ...
        model.save_model('output/universal_construction_model.cbm')
        print(f"\n✅ Universal construction model saved to output/universal_construction_model.cbm")
...
if __name__ == "__main__":
    model, r2_score = universal_construction_model()